PROGRAM run_imaginary_ising_ipeps_FU
  ...
  DO ix=1,nx
     ...
     !! (1) Calc iPEPS environment
     CALL calc_CTM_environment_ipeps(Cmat, Tmat, ipeps, CTMRG, use_old_ctm = read_state .OR. (ix .GT. 1)) 

     !! (2) Full Update TEBD propagation using decreasing values of timestep dt 
     DO j=1,N_tebd
        ...
        CALL time_evolve_full(ipeps, Cmat, Tmat, mpo_prop, TEBD, CTMRG, use_old_ctm = read_state .OR. (ix .GT. 1) .OR. (j .GT. 1))
...
END PROGRAM run_imaginary_ising_ipeps_FU
