PROGRAM run_ising_ipeps
  ...
  INTEGER, PARAMETER :: exit_level(N_tebd) = (/1, 1, 2, 2, 2/)
  ...
  ELSE
     ...
     CALL initialize_rho_iPEPS(iGamma, iLambda, local_dim) !, chi=1, rho_vec=rho_vec)
  end IF
  ...
  DO ix=1,nx
     ...
     CALL read_xname(x, xname, g, "g", kappa, "kappa")

     !! Set initial index
     j0 = 2
     IF(ix .EQ. 1) j0 = 1

     !! TEBD propagation using decreasing values of timestep dt 
     !! (j=N_tebd: final round of TEBD propagation to converge observables, once lambdas have converged)
     DO j=1,2 !N_tebd 
        ...
        !! Create propagator
        CALL setup_propagator(prop, g, V, kappa, 0.0D0, TEBD % dt, 'PEPS')
...
CONTAINS
 ...
 SUBROUTINE dump_parameters(HANDLE)
    ...
    WRITE(HANDLE, '("# g= ", D12.5, " kappa=",D12.5)') g, kappa
 ...
 SUBROUTINE setup_observables()
  ...
  ALLOCATE(TRACE_OUT_1P(5), TRACE_OP_1P(5, hs_dim, hs_dim))

  CALL add_1p_conv_trace(0.5*(eye + sigma_z), "N_up", "time", "N_up", "Trace")
  ...
  ALLOCATE(SUMM_OUT_1P(5), SUMM_OP_1P(5, hs_dim, hs_dim))

  CALL add_1p_summary(0.5*(eye + sigma_z),  "N_up", xname, "N_up", "Trace")
