PROGRAM run_imaginary_bhm_ipeps
  ...
  ELSE
     CALL initialize_psi_iPEPS(iGamma, iLambda, local_dim, psi_vec=SQRT(0.5D0) * (/ (1.0D0, 0.0D0), (-1.0D0, 0.0D0) /))
     !CALL initialize_psi_iPEPS(iGamma, iLambda, local_dim, chi=chi)
  end IF
  ...
  DO ix=1,nx
     ...
     DO ll=1,N_tebd 
        ...
        !! Create propagator
        CALL setup_propagator_dense(prop, g, U, J, TEBD % dt, 'PEPS')
...
END PROGRAM run_imaginary_bhm_ipeps
