MODULE tebd_callback_routines_ipeps
...
CONTAINS
 ...
 !!! Test if Lambdas have converged !!!
 SUBROUTINE test_lambda_convergence_averaged(is_converged, newLambda, oldLambda, tebd, datafile, iter, time)
  ...
  COMPLEX(KIND=DP), ALLOCATABLE :: newLambdaAVG(:) !! average lambdas
  COMPLEX(KIND=DP), ALLOCATABLE :: oldLambdaAVG(:) !! average lambdas
  ...
  !! Test convergence of averaged lambdas
  oldLambdaAVG = average_lambda(oldLambda)
  newLambdaAVG = average_lambda(newLambda)
  CALL test_convergence_vec(is_converged, newLambdaAVG, oldLambdaAVG, etaR, &
                            & datafile, iter, TEBD % test_interval, xval=TEBD % dt, PRINT_ON=.TRUE.)
  
  !! Set old --> old = new for next iter
  CALL copy_lambda_block(oldLambda, newLambda)

 END SUBROUTINE test_lambda_convergence_averaged



 !!! Find error in lambdas !!!
 FUNCTION lambdaErr(lambda, lambda_old) 

  TYPE(block_lambda), INTENT(IN) :: lambda(:)       
  TYPE(block_lambda), INTENT(IN) :: lambda_old(:)
  REAL(KIND=DP)                  :: lambdaErr  
  ...
  !! Allocate array of errors on each site
  ALLOCATE(eta(N_sites))

  !! Get errors on each site
  DO site=1,N_sites
     nval      = MIN(SIZE(lambda(site) % m), SIZE(lambda_old(site) % m))
     eta(site) = vec_error(lambda(site) % m(1:nval), lambda_old(site) % m(1:nval))
  end DO     
 
  !! Return the largest error
  lambdaErr = MAXVAL(eta)

 END FUNCTION lambdaErr
 ...
 !! Auxiliary function: compute average of all lambdas
 FUNCTION average_lambda(lambdas) result(lambda_avg)
  
  TYPE(block_lambda), INTENT(IN)  :: lambdas(:)
  COMPLEX(KIND=DP),   ALLOCATABLE :: lambda_avg(:)
  ...
  !! Number of lambdas
  N_sites = SIZE(lambdas) 

  !! Min dimension of lambdas
  ALLOCATE(dims(N_sites))
  DO site=1,N_sites
     dims(site) = lambdas(site) % ChiDim
  end DO
  min_dim = MINVAL(dims)

  !! Sum all lambdas
  ALLOCATE(lambda_avg(min_dim)); lambda_avg(:) = 0.0D0
  DO site=1,N_sites
     lambda_avg = lambda_avg + lambdas(site) % m(1:min_dim)
  end DO

  !! Find average of lambdas
  lambda_avg(:) = lambda_avg(:) / N_sites

 END FUNCTION average_lambda
...
END MODULE tebd_callback_routines_ipeps
